...
{
    public class GenerateNormalBillNew
    {
        ...
        public GenerateNormalBillNew()
        {
            _cnn = new WaterBillingEntities();
        }

        public DataSet GenerateNormalBill()
        {
            try
            {
                ...
                if(_cnn.OtherChargies.Where(x => x.EffectDate == mEffDate1).ToList().Count > 0)
                {
                    X_DPC = _cnn.OtherChargies.Where(x => x.EffectDate == mEffDate1).FirstOrDefault().DPCPercantage;
                }
                else
                {
                    throw new Exception("Please enter Other Chargies.");
                }
                ...
                decimal mDebitNoteAmount = Convert.ToDecimal(_Obj.DebitNoteAmount);
                decimal mDebitNoteDPCAmount = Convert.ToDecimal(_Obj.DebitNoteDPCAmount);
                decimal mCreaditNoteAmount = Convert.ToDecimal(_Obj.CreaditNoteAmount);
                decimal mCreaditNoteDPCAmount = Convert.ToDecimal(_Obj.CreaditNoteDPCAmount);
                ...
                mPrevRead = Convert.ToInt32(_Obj.PrevRead);
                ...
                string _WhereCondition = " and RefConsumerId = (Select Id from ConsumerMaster Where ConsumerNo = '" + GenerateBill.X_CustNo + "') and Cast(ReceiptDate as Date) >= Cast(" + mPrevIDate + " as Date) and ChqBounceDate Is Null";
                ...
                if (a4xEDt.Count - 1 > 0)
                {
                    for (int i = 0; i < a4xEDt.Count; i++)
                    {
                        ...
                        {
                            a4Months.Add((a4xFDt[i + 1].Year - a4xFDt[i].Year) * 12 + (a4xFDt[i + 1].Month - a4xFDt[i].Month));
                            a4Days.Add(Math.Abs((a4xFDt[i + 1] - a4xFDt[i]).Days));
                        }
                        else
                        {
                            a4Months.Add((mUptoCalDt.Value.Year - a4xFDt[i].Year) * 12 + (mUptoCalDt.Value.Month - a4xFDt[i].Month));
                ...
                if ((GenerateBill.X_PrevStatus == "HLK" && GenerateBill.X_CurrStatus == "MOK") || (GenerateBill.X_PrevStatus == "INV" && GenerateBill.X_CurrStatus == "MOK"))
                {
                ...
                if (mMTypeCode == "BRD")
                {
                    ...
                    foreach (var _Rate1 in _ObjBoardRent.getBoardRentMasterSelectWhere("and Cast(EffectDate as Date) = Cast(" + mEffDate1 + " as Date) and RefMeterSizeId = " + mMSizeId).Select(x => x.Rate).ToList())
                    {
                        ...
                        if (mEffDate1 != mEffDate2)
                        {
                            for (int i = 0; i < a4xEDt.Count; i++)
                            {
                                foreach (var _Rate2 in _ObjBoardRent.getBoardRentMasterSelectWhere("and Cast(EffectDate as Date) = Cast(" + a4xEDt[i] + " as Date) and RefMeterSizeId = " + mMSizeId).Select(x => x.Rate).ToList())
                                {
                ...
                if (mIsAsPerSlab)
                {
                    ...
                    double mMonthlyQtyForRate = Math.Round(Convert.ToDouble(((mQtyToCheck / mDy) * 30) * 1000), 2);
                ...
                else
                {
                    foreach (var _ConsumeRate1 in _ObjConsumerRate.SelectConsumerRateSelectWhere(" and Cast(EffectDate as Date) = Convert(Date,'" + mEffDate1 + "' ,103) and RefSupplyCategoryId = " + mSupCategoryId + " and RefSupplyTypeId = " + mSupTypeId).Select(x => x.Rate).ToList())
                    {
                        ...
                        else
                        {
                            ...
                            for (int i = 0; i < a4xEDt.Count; i++)
                            {
                                foreach (var _ConsumeRate2 in _ObjConsumerRate.SelectConsumerRateSelectWhere(" and Cast(EffectDate as Date) = Cast(" + a4xEDt[i] + " as Date) and RefSupplyCategoryId = " + mSupCategoryId + " and RefSupplyTypeId = " + mSupTypeId).Select(x => x.Rate).ToList())
                                {
                ...
                foreach (var _MeterRate1 in _ObjMeterMinCharge.getMeterMinChargeSelectWhere(_WhereCondition).Select(x => x.Rate).ToList())
                {
                    if (mEffDate1 == mEffDate2)
                    {
                        xRate2 = Convert.ToDouble(_MeterRate1 * mCalMths);
                    }
                    else
                    {
                        xRate2 = 0;
                        for (int i = 0; i < a4xEDt.Count; i++)
                        {
                            _WhereCondition = " and Cast(EffectDate as Date) = Cast(" + a4xEDt[i] + " as Date) and RefSupplTypeId = " + mSupTypeId + " and RefMeterStatusId = " + mStatusId + " and RefMeterSizeId = " + mMSizeId;
                            foreach (var _MeterRate2 in _ObjMeterMinCharge.getMeterMinChargeSelectWhere(_WhereCondition).Select(x => x.Rate).ToList())
                            {
                                xRate2 = xRate2 + Convert.ToDouble(_MeterRate2 * a4Months[i]);
                            }
                        }
                    }
                }
                ...
                if (mOpBal > 0)
                {
                    if (!mIsRcptReceipved)
                    {
                        mDaysForDPC = (mUptoCalDt.Value - mFromCalDt.Value).Days;
                    }
                    else
                    {
                ...
                var _result = Convert.ToBoolean(_ObjConsumeDetail.set_Consumerdetail_save(Id, ConsumerId, Convert.ToInt32(mReaderId),
                        mMSizeId, GenerateBill.X_CurrStatusID, GenerateBill.X_ReadingDate, MeterReading, mBillType, "NorBill",

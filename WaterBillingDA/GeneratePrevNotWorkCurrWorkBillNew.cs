...
{
    public class GeneratePrevNotWorkCurrWorkBillNew
    {
        ...
        public GeneratePrevNotWorkCurrWorkBillNew()
        {
            _cnn = new WaterBillingEntities();
        }

        public void GenerateMeterFixingBills()
        {
            try
            {
                ...
                int mMth1 = 0;
                int mMth2 = 0;
                int mYear1 = mFromCalDt.Value.Year;
                int mYear2 = mUptoCalDt.Value.Year;
                int mDays1 = 0;
                int mDays2 = 0;

                if (mFromCalDt.Value.Day <= 20)
                {
                    mMth1 = mFromCalDt.Value.Month;
                }
                else if (mFromCalDt.Value.Day > 20)
                {
                    ...
                    {
                        mMth1 = mFromCalDt.Value.Month - 1;
                    }
                }

                if (mUptoCalDt.Value.Day <= 20)
                {
                    mMth2 = mUptoCalDt.Value.Month;
                }
                else if (mUptoCalDt.Value.Day > 20)
                {
                    if (mUptoCalDt.Value.Month == 1)
                    {
                        mMth2 = 12;
                        mYear2 = mYear2 + 1;
                    }
                    else if (mUptoCalDt.Value.Month > 1)
                    {
                        mMth2 = mUptoCalDt.Value.Month + 1;
                    }
                }

                int mDiff = (mUptoCalDt.Value.Year - mFromCalDt.Value.Year) * 12 + (mUptoCalDt.Value.Month - mFromCalDt.Value.Month);
                int mCalMths = (mUptoCalDt.Value.Year - mFromCalDt.Value.Year) * 12 + (mUptoCalDt.Value.Month - mFromCalDt.Value.Month);
                ...
                if (!GenerateBill.X_CalAsPerDays)
                    mQtyCons = mQtyCons != 0 ? mQtyCons : Convert.ToInt32(Math.Round(Convert.ToDouble(mAvg * mCalMths), 0));
                else
                    mQtyCons = mQtyCons != 0 ? mQtyCons : Convert.ToInt32(Math.Round(Convert.ToDouble(((double)mAvg / (double)30)) * mDays1, 0));

                string mCMth = mMth1.ToString("MMM") + " " + mYear1.ToString() + " - " + mMth2.ToString("MMM") + " " + mYear2.ToString();
                int mTAvg = 0;

                if (!GenerateBill.X_CalAsPerDays)
                    mTAvg = mAvg * mCalMths;
                else
                    mTAvg = Convert.ToInt32(Math.Round(Convert.ToDouble((double)mAvg / (double)30) * mDays1, 0));
                ...
                if (_Obj.WrkSince == null)
                {
                    mDaysDiff1 = (mUptoCalDt.Value - mFromCalDt.Value).Days - 1;
                    mDaysDiff2 = 0;
                }
                else
                {
                    mDaysDiff1 = (_Obj.WrkSince.Value - mFromCalDt.Value).Days - 1;
                    mDaysDiff2 = (mUptoCalDt.Value - _Obj.WrkSince.Value).Days;
                }
                ...
                if (!GenerateBill.X_CalAsPerDays)
                    mAQty1 = Convert.ToInt32(Math.Round(Convert.ToDouble(mAvg * mCalMths), 0));
                else
                    mAQty1 = Convert.ToInt32(Math.Round(Convert.ToDouble(mAvg * mDaysDiff1) / 30, 0));
                ...
                foreach (var _ConsumeRate1 in _ObjConsumerRate.SelectConsumerRateSelectWhere(" and Cast(EffectDate as Date) = Convert(Date,'" + mEffDate1 + "' ,103) and RefSupplyCategoryId = " + mSupCategoryId + " and RefSupplyTypeId = " + mSupTypeId).Select(x => x.Rate).ToList())
                {
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < a4xEDt.Count; i++)
                        {
                            ...
                            {
                                if (!GenerateBill.X_CalAsPerDays)
                                    xR = xR + Convert.ToInt32(_ConsumeRate2 * a4Days[i]);
                                else
                        ...
                        if (!GenerateBill.X_CalAsPerDays)
                            xRate1 = Convert.ToInt32(Math.Round(Convert.ToDouble(xR / mM), 2));
                        else
                ...
                //int mNWorkStatID = 0;
                //int mStatusId = 0;
                //mWorkStatId = _ObjMastervalue.getMasterValueWhere(6, " and ShortName = 'MOK'").FirstOrDefault().ID;
                //mNWorkStatID = _ObjMastervalue.getMasterValueWhere(6, " and ShortName = 'NWK'").FirstOrDefault().ID;
                ...
                //        mQty + mQty2, mBookNo, mNoOfFlats, InsUser, InsTerminal, UpdUser, UpdTerminal));
        ...
        public int GetPrevHLQtyNew(string _ConsumerNo)
        {
            ...
            try
            {
                int _MeterStatusId = _cnn.sp_MasterValue_SelectWhere(6, " and ShortName = 'HLK'").FirstOrDefault().ID;
